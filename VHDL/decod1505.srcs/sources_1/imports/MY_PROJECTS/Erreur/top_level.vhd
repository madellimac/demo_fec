...
--signal decoded_bit, decode_valid : std_logic;
...
--signal valid_reg,s_decoded      : std_logic := '0';
...
--    -- === Bit Accumulation and RAM Write Process ===
--    process(clk)
--    begin
--        if rising_edge(clk) then
--            if rst = '1' then
--                decoded_byte     <= (others => '0');
--                bit_cnt          <= 0;
--                write_ptr        <= 0;
--                last_written_ptr <= 0;
--                valid_reg        <= '0';
...
--            elsif decode_valid = '1' and latch_bit = '0' then
--                -- latch current bit into decoded_byte
--                decoded_byte(bit_cnt) <= decoded_bit;
--                latch_bit <= '1'; -- prevent double-latching if decode_valid is high multiple cycles

--                if bit_cnt = 7 then
--                    bit_cnt <= 0;
--                    decoded_ram(write_ptr) <= decoded_byte;
--                    last_written_ptr       <= write_ptr;
--                    write_ptr              <= (write_ptr + 1) mod RAM_DEPTH;
--                    valid_reg              <= '1';  -- valid byte ready
--                else
--                    bit_cnt <= bit_cnt + 1;
--                    valid_reg <= '0';
--                end if;

--            elsif decode_valid = '0' then
--                latch_bit <= '0';  -- allow next decode_valid to be latched
--                valid_reg <= '0';
--            end if;
--        end if;
--    end process;
...
--    -- Outputs
--    decoded_ram_out <= decoded_ram(last_written_ptr);
--    result          <= decoded_bit;
--    decode_ready    <= decode_valid;
